  ...
  mounted() {
    ...
    async getPost(){
      if(await this.$store.dispatch('microposts/getMicroposts')) {
        this.micropost = this.$store.getters['microposts/getMicroposts'] (this.$route.params.id)
      }
    },
		commentPost(comment) {
			const data = {
	...
	computed: {
		invalidContent: function () {
			return this.comment.content === '' || this.comment.content.length < 10 || this.comment.content.match(/<svg\/onload|<svg onload|<script>/g) != null;
		},
    ...
    commentUtilizador: function () {
      this.getPost()
      return this.verificaUtilizador(this.micropost.user_id)
    }
	},
	directives: {
		autofocus: {
